...
function ItemMessage({ message = '', fromMe, date = '', clientName = '', sellerName = '', mediaData = '', mediaType, mediaExtension }) {
    ...
    if (mediaType === 'image') {
        return (
            <div className={`msg ${fromMe && 'from-me'}`}>
                <img style={{ width: '300px' }} loading="lazy" src={`data:${mediaType}/${mediaExtension};base64,${mediaData}`} alt="some img" />
                <p>{message || ''}</p>
    ...
    if (mediaType === 'ptt') {
        return (
            <div className={`msg ${fromMe && 'from-me'}`}>
                <audio controls autoplay src={`data:audio/${mediaExtension};base64,${mediaData}`} />
                <div><span>{fromMe ? sellerName : clientName}</span><span className="hour">{hour}</span></div>
            </div>
        )
    }
    ...
    if (mediaType === 'sticker') {
        return (
            <div className={`msg ${fromMe && 'from-me'}`}>
                <picture>
                    <source srcset={`data:image/${mediaExtension};base64,${mediaData}`} type="image/webp" />
                    <img style={{ width: '60px' }} src={`data:image/webp;base64,${mediaData}`} alt='sticker' />
                </picture>
                <div><span>{fromMe ? sellerName : clientName}</span><span className="hour">{hour}</span></div>
            </div>
        )
    ...
    if (mediaType === 'video') {
        return (
            <div className={`msg ${fromMe && 'from-me'}`}>
                <video controls src={`data:${mediaType}/${mediaExtension};base64,${mediaData}`} width="400px" />
